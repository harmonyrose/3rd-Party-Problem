# -*- coding: utf-8 -*-
"""
Created on Fri Apr  7 01:12:21 2023
...
class Voter(Agent):
    ...
    def step(self):
        ...
        # get all agents' opinions on i1
        agts = self.model.schedule.agents
        agt_ops = []
        for agt in agts:
            agt_ops.append(agt.opinions)
        ...
        for i in range(len(agt_ops)):
            for j in range(len(agt_ops[i])):
                agt_ops[i][j] -= self.opinions[j] 
                agt_ops[i][j] = 1-abs(agt_ops[i][j])

        agt_ops = np.array(agt_ops)
        s_ops = np.array(self.opinions)
        agt_dists = []
        for i in range(len(agt_ops)):
            agt_dists.append(np.linalg.norm(s_ops - agt_ops[i]))
        
        
        agt_weights = agt_dists/np.sum(agt_dists)
