# -*- coding: utf-8 -*-
"""
Created on Fri Apr  7 00:30:40 2023
...
class Voter(Agent):
    ...
    def step(self):
        ...
        # get the opinions of all agents in the graph on i1
        agts = self.model.schedule.agents
        agt_ops = []
        for agt in agts:
            agt_ops.append(agt.opinions[i1])
        
        # calculate difference between self's and agents' opinions
        # on i1, then divide each one by the sum of all opinions to get
        # probability weights
        agt_ops-=(self.opinions[i1])
        agt_ops = 1-abs(agt_ops)
        agt_weights = agt_ops/np.sum(agt_ops)
        
        # choose an agent to interact with; agents with similar opinions
        # on i1 have a higher chance of being chosen
        chosen_agt = np.random.choice(agts, p=agt_weights)
        
        # calcuate the difference between self and agent's opinion
        diff = abs(self.opinions[i1] - chosen_agt.opinions[i1])
        ...
        if diff <= openness:
            self.opinions[i2] = (self.opinions[i2] + chosen_agt.opinions[i2])/2
            opinion_moved = True
